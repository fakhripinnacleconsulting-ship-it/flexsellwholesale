 ...
 *
 * Organised as disclosure sections matching the staff wallet panel so that the admin and customer
...
export default function ClientWalletPage() {
  ...
  // Manage accordions independently so all can be open at once (matching StaffWalletPanel)
  ...
  const loadBreakdown = React.useCallback(async () => {
    ...
    try {
      const data = await walletService.getBreakdown({ walletType: breakdownTab, ...dates });
      setBreakdown(data);
    } catch (err) {
  ...
  const handleTransferConfirm = async (amount: number, password?: string) => {
    await walletService.transferToBusinessWallet({
      userId: auth.customer?._id || "",
      amount,
      adminPassword: password,
    });
    addToast(`${formatPrice(amount)} moved to your Business Wallet.`, "success");
          ...
          filters={
            <div className="flex flex-wrap items-center justify-end gap-2">
              ...
              <a
                ...
                href={walletService.statementUrl({ walletType: passbookTab, from: range.from, to: range.to })}
