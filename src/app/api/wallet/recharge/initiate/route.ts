...
import { createPendingRecharge } from "@/lib/walletRecharge";
...
export async function POST(request: NextRequest) {
  try {
    ...
    /**
     * Customers top up their own wallet; **admins** may take an online payment on a
    ...
    if (payload.role !== "customer" && !isAdmin) {
      return NextResponse.json(
        { message: "Only the account holder or an admin can start a wallet top-up" },
        { status: 403 }
      );
    }
     ...
     */
    const availability = await getWalletRechargeAvailability();
    ...
    /**
     * Whose wallet is being topped up.
     *
    ...
    if (isAdmin && !body.userId) {
      return NextResponse.json(
        { message: "Choose which customer's wallet to top up" },
        { status: 400 }
      );
    }
    ...
    if (body.termsAccepted !== true) {
      return NextResponse.json(
        { message: "Please accept the wallet terms before adding money" },
        { status: 400 }
      );
    }
    ...
    // Admins are Customer documents with role "admin"; they are staff, not wallet holders.
    ...
    if (walletType === "business") {
      ...
      if (!eligible) {
        return NextResponse.json(
          { message: "The Business Wallet is available to B2B and Dropshipping accounts" },
          { status: 403 }
        );
      }
    ...
    if (!key_id || !key_secret) {
      console.error("[Wallet] Razorpay keys are missing from environment variables.");
     ...
     */
    const actor = isAdmin
      ? (resolveActor(payload) as WalletActor)
      : (resolveActor(payload, customer.name) as WalletActor);
    ...
    try {
      ...
      console.error("[Wallet] Razorpay order creation failed:", err);
    ...
    console.error("[Wallet] Recharge initiate failed:", error);
    return NextResponse.json(
      { message: (error as Error).message || "Failed to start wallet recharge" },
      { status: 500 }
    );
  }
}
