...
import { ADVANCE_BALANCE_TYPES, ADVANCE_BALANCE_TERMS_TEXT } from "@/lib/advanceBalanceConstants";

export const dynamic = "force-dynamic";

/**
 * Downloads a Advance Balance statement as CSV.
 *
 ...
 * formula to Excel, not text, and Advance Balance descriptions are free text written by staff.
 */
export async function GET(request: NextRequest) {
  try {
    ...
    /**
     * Accepts `all` alongside the two advanceBalances — the third place this had to change.
     *
    ...
    // Normalised to undefined so every query below reads "no Advance Balance filter" without repeating
    // the "all" special case at each site.
    const walletType = advanceBalanceTypeParam === "all" ? undefined : advanceBalanceTypeParam;
    ...
    const [rows, categories, customer, openingRow] = await Promise.all([
      ...
      AdvanceBalanceTransaction.find(query).sort({ createdAt: 1 }).limit(5000).lean(),

      AdvanceBalanceExpenseCategory.find({}).select("key label").lean() as Promise<
    ...
    const meta = [
      ["FlexSell Wholesale — Advance Balance Statement"],
      ["Customer", customer?.name || userId],
      ["Email", customer?.email || ""],
      ["Wallet", walletType === "business" ? "Business Advance Balance" : walletType === "store" ? "Store Advance Balance" : "All wallets"],
    ...
    const csv = [
      ...
      "",
      toCsvRow([ADVANCE_BALANCE_TERMS_TEXT]),
    ].join("\r\n");
    ...
    const filename = `Advance_Balance_Statement_${cleanName}_${period}.csv`;
    ...
    console.error("[advanceBalance] Statement export failed:", error);
 ...
 * make a spreadsheet treat a cell as a formula, and Advance Balance descriptions are free text typed
