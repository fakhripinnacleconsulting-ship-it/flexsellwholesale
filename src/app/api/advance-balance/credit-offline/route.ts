...
import type { AdvanceBalanceTransactionSource } from "@/types/advanceBalance";
...
/**
 * Credits a Advance Balance for money received outside the payment gateway.
 *
...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      const { transaction, advanceBalance } = await runInTransaction((session) =>
        writeLedgerEntry({
          ...
          walletType,
          type: "CREDIT",
          source: source as AdvanceBalanceTransactionSource,
          amountPaise,
    ...
    console.error("[advanceBalance] Offline credit failed:", error);
    return NextResponse.json(
    ...
    dispatchEventServer({
      ...
      data: {
        ...
        message: params.pending
          ? `${formatPaise(params.amountPaise)} received by ${SOURCE_LABEL[params.source].toLowerCase()} has been recorded against your ${advanceBalanceLabelText}. It becomes available once cleared.`
          : `${formatPaise(params.amountPaise)} received by ${SOURCE_LABEL[params.source].toLowerCase()} has been added to your ${advanceBalanceLabelText}.`,
      },
    });
  } catch (err) {
    console.error("[advanceBalance] Failed to notify customer of offline credit:", err);
  }
}
