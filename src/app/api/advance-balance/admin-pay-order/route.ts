...
/**
 * Pays for an existing order from the customer's Advance Balance on behalf of them by an Admin/Manager.
 *
 * This endpoint allows staff to settle an order using the customer's Store or Business Advance Balance.
 */
export async function POST(request: NextRequest) {
  try {
    ...
    if (walletType !== "store" && walletType !== "business") {
      return NextResponse.json({ message: "Invalid Advance Balance type" }, { status: 400 });
    }

    /**
     * The permission is derived from `walletType`, so a `wallet_business` holder cannot reach
     * a Store Advance Balance by changing a field in the payload.
     *
     ...
     * gated on the exact Advance Balance permission and nothing else.
     */
    const auth = await requireAdvanceBalanceSpendAccess(walletType);
    ...
    // The Advance Balance being charged must belong to the customer this order is for. Without this a
    ...
    try {
      const captured = await captureAdvanceBalanceFunds({ holdId: hold.holdId, orderId });
      ...
      const updatedOrder = await Order.findOneAndUpdate(
        { _id: orderId, paymentStatus: { $ne: "Paid" }, status: { $ne: "Cancelled" } } as any,
        {
          $set: {
            ...
            // Which Advance Balance paid, kept alongside the method rather than encoded into it: the
            // method drives display and reporting, and collapsing both advanceBalances into one
            // string is what made a failed Business Advance Balance payment un-retryable.
            walletType,
            paymentStatus: "Paid",
       ...
       * the same reason as /api/advance-balance/pay-order: that handler refunds the capture, and the
      ...
      try {
        ...
        console.error(
          `[advanceBalance] Order ${orderId} is paid but its tax invoice could not be issued:`,
          docErr
        );
      }
      ...
      return NextResponse.json(
        {
          message: `Paid from ${walletType === "business" ? "Business Advance Balance" : "Store Advance Balance"}`,
          orderId,
          transactionId: captured.transactionId,
       ...
       * checkout.
       * 
       * Because capture succeeded, we must use `refundAdvanceBalanceOrder` (which refunds a successful capture)
       * rather than `releaseAdvanceBalanceFunds` (which only releases pending holds).
       */
      if (capturedTxId) {
        await refundAdvanceBalanceOrder({
          ...
          console.error("[advanceBalance] Failed to refund Advance Balance after a failed order capture:", refundErr)
        );
      }
      throw err;
    }
  } catch (error: unknown) {
    console.error("[advanceBalance] Order payment failed:", error);
    return NextResponse.json(
      { message: (error as Error).message || "Could not pay from your Advance Balance" },
      { status: 500 }
    );
  }
}
