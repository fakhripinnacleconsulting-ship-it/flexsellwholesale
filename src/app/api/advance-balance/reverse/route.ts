...
import { nextReceiptNumber, isCreditType } from "@/lib/advanceBalanceLedger";
import { ADMIN_REAUTH_THRESHOLD_PAISE } from "@/lib/advanceBalanceConstants";

export const dynamic = "force-dynamic";

/**
 * Reverses a Advance Balance entry.
 *
...
export async function POST(request: NextRequest) {
  try {
    ...
    const original = await AdvanceBalanceTransaction.findById(transactionId).lean() as
      | {
          _id: string;
          walletId: string;
          userId: string;
    ...
    const result = await runInTransaction(async (session) => {
       ...
       */
      const advanceBalance = originalWasCredit
      ...
      const [reversal] = await AdvanceBalanceTransaction.create(
        [
          {
            ...
            balanceBefore: originalWasCredit
              ? advanceBalance.availableBalance + original.amount
              : advanceBalance.availableBalance - original.amount,
      ...
      return { reversalId: String(reversal._id), balancePaise: advanceBalance.availableBalance };
    });
    ...
    console.error("[advanceBalance] Reversal failed:", error);
    return NextResponse.json(
