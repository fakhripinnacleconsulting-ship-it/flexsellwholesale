...
} from "@/lib/advanceBalanceLedger";
import { runInTransaction } from "@/lib/transactionHelper";
import { ADVANCE_BALANCE_TYPES, BUSINESS_ADVANCE_BALANCE_TIERS } from "@/lib/advanceBalanceConstants";
...
export async function POST(request: NextRequest) {
  try {
    ...
    // The permission is chosen from the Advance Balance being written, never from anything else in
    // the body — otherwise a wallet_business holder could reach a Store Advance Balance by changing
    // one field.
    ...
    const [customer, category] = await Promise.all([
      Customer.findById(userId).select("name email role customerTypes upgradeStatus").lean() as Promise<
        ...
        | null
      >,
      AdvanceBalanceExpenseCategory.findOne({ key: expenseCategory }).select("label isActive").lean() as Promise<
    ...
    if (walletType === "business") {
      ...
      if (customer.upgradeStatus !== "approved") {
        return NextResponse.json(
          {
            message:
              "This customer's KYC is not approved yet. Services cannot be charged to the Business Advance Balance until it is.",
    ...
    try {
      const { transaction, advanceBalance } = await runInTransaction((session) =>
        writeLedgerEntry({
          userId,
          walletType,
          type: "DEBIT",
      ...
      void emailExpense({
        ...
        balanceAfterPaise: advanceBalance.availableBalance,
      ...
      if (err instanceof InsufficientBalanceError || err instanceof AdvanceBalanceNotActiveError) {
        return NextResponse.json({ message: err.message }, { status: err.status });
      }
      throw err;
    }
  } catch (error: unknown) {
    console.error("[advanceBalance] Expense failed:", error);
    return NextResponse.json(
 ...
 * spending, which no single customer's Advance Balance can provide.
 *
...
async function emailExpense(params: {
  ...
  const advanceBalanceLabelText = params.walletType === "business" ? "Business Advance Balance" : "Store Advance Balance";
  ...
  try {
    ...
    if (params.customerEmail) {
      tasks.push(
        emailService.sendAdvanceBalanceExpenseEmail({
          to: params.customerEmail,
          customerName: params.customerName,
          amount: toRupees(params.amountPaise),
          advanceBalanceLabelText,
    ...
    tasks.push(
      emailService.sendAdminAdvanceBalanceExpenseAlert({
        customerName: params.customerName,
    ...
    console.error("[advanceBalance] Failed to email expense notifications:", err);
  }
}
  ...
  try {
    ...
    dispatchEventServer({
      ...
      data: {
        ...
        message: `${formatPaise(params.amountPaise)} was spent from your ${advanceBalanceLabelText} on ${params.categoryLabel} by ${params.actorName}.`,
      },
    });
  } catch (err) {
    console.error("[advanceBalance] Failed to notify customer of expense:", err);
  }
}
