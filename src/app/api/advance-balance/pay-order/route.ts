...
import { reserveAdvanceBalanceFunds, captureAdvanceBalanceFunds, releaseAdvanceBalanceFunds, refundAdvanceBalanceOrder } from "@/lib/advanceBalanceCheckout";
...
/**
 * Pays for an existing order from the customer's Store Advance Balance.
 *
 * The **customer-facing** entry point, and it accepts the Store Advance Balance only. The Business
 * Advance Balance is services-only from the customer's side; staff placing an order on someone's
  ...
  try {
    ...
    if (payload.role !== "customer") {
      return NextResponse.json(
        { message: "Only the account holder can pay from their own Advance Balance here" },
        { status: 403 }
      );
    }
    ...
    const customer = await Customer.findById(payload.userId).select("name").lean() as
      | { name?: string }
      | null;
    const actor = resolveActor(payload, customer?.name) as AdvanceBalanceActor;
    ...
    try {
      ...
      return NextResponse.json(
        {
          message: "Paid from your Store Advance Balance",
          orderId,
          transactionId: captured.transactionId,
