 ...
 * dropped every order paid from the Advance Balance — those carry an `orderId` and no category, because
...
vi.mock("@/models/AdvanceBalanceExpenseCategory", () => ({
  default: { find: () => ({ select: () => ({ lean: () => Promise.resolve([
    { key: "gst_registration", label: "GST Registration", colour: "#10b981" },
  ]) }) }) },
}));

const mockAdvanceBalanceRead = vi.fn();
vi.mock("@/lib/advanceBalanceGuard", () => ({
  requireAdvanceBalanceRead: (...a: unknown[]) => mockAdvanceBalanceRead(...a),
}));
...
async function call(params: string) {
  const res = await GET(new Request(`http://localhost/api/advance-balance/breakdown${params}`) as never);
...
describe("GET /api/advance-balance/breakdown", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mockAdvanceBalanceRead.mockResolvedValue({ payload: { userId: "CUST-1", role: "customer" } });
  ...
  describe("the All scope", () => {
    ...
    it("excludes transfers, because moving money between your own advanceBalances is not spending it", async () => {
      await call("?walletType=all");
      // Counting the transfer here would double-count: whatever the Business Advance Balance then
    ...
    it("is the default when no Advance Balance is named", async () => {
      const { body } = await call("");
      expect(body.walletType).toBe("all");
    });
  });

  it("still rejects a Advance Balance type that does not exist", async () => {
