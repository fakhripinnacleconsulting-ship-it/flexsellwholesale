...
import { requireAdvanceBalanceAdmin } from "@/lib/advanceBalanceGuard";
import { requireAdminOrManagerAuth } from "@/lib/authGuard";
import { SEED_EXPENSE_CATEGORIES } from "@/lib/advanceBalanceConstants";
...
async function ensureSeeded() {
  const count = await AdvanceBalanceExpenseCategory.estimatedDocumentCount();
  if (count > 0) return;

  await AdvanceBalanceExpenseCategory.insertMany(
...
export async function GET(request: NextRequest) {
  try {
    ...
    const categories = await AdvanceBalanceExpenseCategory.find(query)
    ...
    console.error("[advanceBalance] Category fetch failed:", error);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const existing = await AdvanceBalanceExpenseCategory.findOne({ key }).lean();
    if (existing) {
      return NextResponse.json({ message: "That category key already exists" }, { status: 409 });
    }

    const created = await AdvanceBalanceExpenseCategory.create({
      key,
    ...
    console.error("[advanceBalance] Category create failed:", error);
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    const updated = await AdvanceBalanceExpenseCategory.findOneAndUpdate({ key }, { $set: updates }, { new: true });
    ...
    console.error("[advanceBalance] Category update failed:", error);
