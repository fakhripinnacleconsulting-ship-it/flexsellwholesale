 ...
 */
const WalletActorSchema = new Schema(
  {
    userId: { type: String },
...
const WalletTransactionSchema = new Schema(
  {
    ...
    /** Denormalised from the wallet so per-type queries need no join. */
    ...
    /** Controlled key from WalletExpenseCategory. Required when source is "expense". */
    ...
    createdBy: { type: WalletActorSchema, required: true },
...
WalletTransactionSchema.index({ walletId: 1, expenseCategory: 1 });
...
WalletTransactionSchema.index({ clientRequestId: 1 }, { unique: true, sparse: true });

WalletTransactionSchema.index({ receiptNumber: 1 }, { unique: true });
...
WalletTransactionSchema.index({ "metadata.razorpayOrderId": 1 }, { sparse: true });
