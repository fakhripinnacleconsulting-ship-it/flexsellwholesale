/**
 * What a Advance Balance surface may *draw*, for one viewer.
 *
 ...
 * Every action still passes `requireAdvanceBalanceSpendAccess` / `requireAdvanceBalanceAdmin` on the server
 * ([lib/advanceBalanceGuard.ts](src/lib/advanceBalanceGuard.ts)). Hiding a button is a courtesy to the
 * viewer, not a security boundary — a hidden button that was somehow clicked would still be
 * refused server-side.
 */

export interface AdvanceBalanceCapabilities {
  /** Top up by card/UPI — the customer for their own Advance Balance, staff on a customer's behalf. */
  canAddMoneyOnline: boolean;
  /** Record money received as cash, bank transfer, UPI or cheque. Admin only. */
  canAddFundsOffline: boolean;
  /** Spend a customer's balance on a service. Admin, or a manager with the Advance Balance grant. */
 ...
 */
export function customerCapabilities(options: { onlineRechargeAvailable: boolean }): AdvanceBalanceCapabilities {
  return {
 ...
 *
 * The split is deliberate and mirrors `advanceBalanceGuard`: a manager may **spend** a balance that
 * already exists, but only an admin may **create money** (offline credit), **move it**
 * between advanceBalances, **return it** (reversal) or **stop it** (freeze). Those four have no
