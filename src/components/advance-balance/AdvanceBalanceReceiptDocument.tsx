...
interface AdvanceBalanceReceiptDocumentProps {
  transaction: AdvanceBalanceTransactionView;
  customerName: string;
...
/**
 * A printable receipt for one Advance Balance transaction.
 *
 ...
 */
export function AdvanceBalanceReceiptDocument({
  transaction,
  customerName,
  customerEmail,
  ...
  sellerAddress,
  sellerGstin,
}: AdvanceBalanceReceiptDocumentProps) {
  const isCredit = transaction.direction === "credit";
  const advanceBalanceLabelText = transaction.walletType === "business" ? "Business Advance Balance" : "Store Advance Balance";

  const rows: Array<[string, string | undefined]> = [
    ["Receipt Number", transaction.receiptNumber],
    ["Transaction ID", transaction._id],
    ["Date & Time", formatFullIST(transaction.createdAt)],
    ["Wallet", advanceBalanceLabelText],
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between border-b-2 border-[#10b981] pb-4">
        ...
        <div className="text-right">
          <h2 className="text-sm font-bold uppercase tracking-widest text-[#10b981]">
            Advance Balance Transaction Receipt
          </h2>
      ...
      <div className="mt-8 border-t border-[#e2e8f0] pt-4 text-[9px] leading-relaxed text-[#64748b]">
        <p className="font-semibold">{ADVANCE_BALANCE_TERMS_TEXT}</p>
        <p className="mt-2">
          This is a computer-generated receipt for a Advance Balance transaction and does not require a
