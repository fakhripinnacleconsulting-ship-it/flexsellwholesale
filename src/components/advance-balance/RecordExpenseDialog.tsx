...
import { X, Loader2, ShieldAlert, Wallet as AdvanceBalanceIcon, Briefcase } from "lucide-react";
import type { AdvanceBalanceType, AdvanceBalanceExpenseCategory } from "@/types/advanceBalance";
...
export function RecordExpenseDialog({
  ...
  const [categories, setCategories] = React.useState<AdvanceBalanceExpenseCategory[]>([]);
  ...
  const submit = async () => {
    ...
    try {
      const result = await advanceBalanceService.recordExpense({
        userId,
        walletType,
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    confirmAction({
      ...
      } Advance Balance immediately and they will be notified. It can only be undone by an admin reversal.`,
