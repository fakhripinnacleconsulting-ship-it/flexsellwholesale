...
import { Wallet as AdvanceBalanceIcon, Briefcase, AlertTriangle } from "lucide-react";
import type { AdvanceBalanceView } from "@/types/advanceBalance";

interface AdvanceBalanceCardProps {
  type: "store" | "business";
  advanceBalance: AdvanceBalanceView | null;
  /** Rendered under the figures — the Add Money button, or nothing for staff views. */
  actions?: React.ReactNode;
  /** Shown above the figures. Used for the KYC banner on the Business Advance Balance. */
...
const COPY = {
  store: {
    title: "Store Advance Balance",
    blurb: "For orders and services on FlexSell",
    Icon: AdvanceBalanceIcon,
  },
  business: {
    title: "Business Advance Balance",
 ...
 * A Advance Balance is an accounting document a business owner has to trust, and the visual
 * language that sells a consumer app works against that here.
 */
export function AdvanceBalanceCard({ type, advanceBalance, actions, notice, customerName }: AdvanceBalanceCardProps) {
  const { title, blurb, Icon } = COPY[type];

  /**
   * A missing Advance Balance is not an error and not a zero balance — it is a Advance Balance that has never
   ...
   */
  const available = advanceBalance?.availableBalance ?? 0;
  const held = advanceBalance?.heldBalance ?? 0;
  const credited = advanceBalance?.totalCredited ?? 0;
  const debited = advanceBalance?.totalDebited ?? 0;
  const isFrozen = advanceBalance?.status === "frozen";
  const isClosed = advanceBalance?.status === "closed";
    ...
    <Card className="border border-border">
      <CardContent className="p-5">
        <div className="flex items-start justify-between gap-3 mb-4">
          ...
          {(isFrozen || isClosed) && (
            <Badge variant="destructive" className="text-[10px] uppercase shrink-0">
              {advanceBalance?.status}
            </Badge>
          )}
        </div>

        {/*
          A frozen or closed Advance Balance must explain itself. Without this the customer sees a
        ...
        {(isFrozen || isClosed) && (
          <div className="mb-4 rounded-md border border-destructive/30 bg-destructive/5 px-3 py-2.5">
            <p className="text-[11px] leading-relaxed text-destructive">
              {isFrozen
                ? "This Advance Balance is temporarily frozen. Your balance is safe and your statement stays available, but no money can move in or out."
                : "This Advance Balance is closed. Your transaction history remains available."}
              {advanceBalance?.closureReason && (
                <span className="mt-1 block font-semibold">Reason: {advanceBalance?.closureReason}</span>
              )}
            </p>
          </div>
        )}
        ...
        */}
        ...
        {advanceBalance?.isLowBalance && available > 0 && (
