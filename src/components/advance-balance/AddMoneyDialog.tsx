...
import { ADVANCE_BALANCE_TERMS_TEXT } from "@/lib/advanceBalanceConstants";
import { X, ShieldAlert, Loader2 } from "lucide-react";
import type { AdvanceBalanceType } from "@/types/advanceBalance";
...
interface AddMoneyDialogProps {
  ...
  /**
   * When provided, the dialog offers a Advance Balance selector.
   *
   * The staff panel opens this from one button rather than one per Advance Balance, so the choice has
   * to live inside — matching how the expense and offline-funds dialogs already work. Absent
   * for customers, who arrive from a specific Advance Balance card and have already chosen.
   */
  allowAdvanceBalanceChange?: (next: AdvanceBalanceType) => void;
}
...
export function AddMoneyDialog({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const session = await advanceBalanceService.initiateRecharge({
        walletType,
      ...
      await openRazorpayCheckout({
        ...
        description: walletType === "business" ? "Business Advance Balance top-up" : "Store Advance Balance top-up",
        ...
        handler: async (response: {
          ...
          try {
            await advanceBalanceService.verifyRecharge(response);
            addToast(isAssisted ? "Payment collected and credited." : "Money added to your Advance Balance.", "success");
            onCredited();
          } catch {
            /**
        ...
        <div className="mb-4 flex items-start justify-between gap-3">
          <div>
            ...
            <p className="text-[11px] text-muted-foreground">
              {isAssisted
                ? `into ${onBehalfOf!.customerName}'s ${walletType === "business" ? "Business" : "Store"} Advance Balance`
                : `to your ${walletType === "business" ? "Business" : "Store"} Advance Balance`}
            </p>
          </div>
          <button
            type="button"
          ...
          {allowAdvanceBalanceChange && (
            <div>
              ...
              <select
                id="wallet-target"
                value={walletType}
                onChange={(e) => allowAdvanceBalanceChange(e.target.value as AdvanceBalanceType)}
                className="h-10 w-full cursor-pointer rounded-md border bg-background px-3 text-sm font-semibold text-foreground"
              >
                <option value="store">Store Advance Balance — products and services</option>
                <option value="business">Business Advance Balance — services only</option>
              </select>
            </div>
          )}
