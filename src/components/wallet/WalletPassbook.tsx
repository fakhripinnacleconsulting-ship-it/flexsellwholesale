...
import type { WalletStatementPage, WalletTransactionView } from "@/types/wallet";

interface WalletPassbookProps {
  data: WalletStatementPage | null;
  isLoading?: boolean;
  error?: string | null;
  onPageChange?: (page: number) => void;
  onRaiseQuery?: (transactionId: string, label: string) => void;
  onViewReceipt?: (transaction: WalletTransactionView) => void;
      ...
      <Body className={bare ? "overflow-hidden rounded-lg border" : "p-0"}>
        {isLoading ? (
          ...
          <EmptyState
            ...
            description="Money added to and spent from this wallet will appear here."
          />
        ) : (
          <>
            ...
            <div className="overflow-x-auto">
              <table className="w-full min-w-[46rem] border-collapse text-left">
                <caption className="sr-only">
                  Wallet transactions{data?.from ? ` from ${data.from} to ${data.to}` : ""}, showing
