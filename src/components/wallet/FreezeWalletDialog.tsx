...
export function FreezeWalletDialog({ walletType, isFreezing, open, onClose, onConfirm }: FreezeWalletDialogProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.message || `Failed to ${isFreezing ? "freeze" : "reactivate"} wallet`);
    } finally {
  ...
  return (
    <Dialog open={open} onOpenChange={(val) => !val && onClose()}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>{isFreezing ? `Freeze ${walletType} wallet?` : `Reactivate ${walletType} wallet?`}</DialogTitle>
          <DialogDescription>
            {isFreezing 
        ...
        <form onSubmit={handleSubmit} className="space-y-4 py-4">
          {isFreezing && (
            <div className="space-y-2">
              <label className="text-sm font-medium">Why is this wallet being frozen?</label>
              <textarea
                value={reason}
          ...
          <DialogFooter>
            ...
            <Button type="submit" variant={isFreezing ? "destructive" : "default"} disabled={isSubmitting || (isFreezing && reason.trim().length < 5)}>
              {isSubmitting ? "Processing..." : (isFreezing ? "Freeze Wallet" : "Reactivate")}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
