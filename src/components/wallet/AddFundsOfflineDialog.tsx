...
import { X, Loader2, AlertTriangle, Wallet, Briefcase } from "lucide-react";
import type { WalletType, OfflineCreditInput } from "@/types/wallet";
...
export function AddFundsOfflineDialog({
  ...
  const submit = async () => {
    ...
    try {
      const result = await walletService.creditOffline({
        userId,
        walletType,
        source,
