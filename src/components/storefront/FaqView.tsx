...
export function FaqView({ initialFaqs }: { initialFaqs?: FaqItem[] }) {
  const [openIdx, setOpenIdx] = React.useState<number | null>(0);
  const [faqItems, setFaqItems] = React.useState<FaqItem[]>(initialFaqs || []);
  const [isLoading, setIsLoading] = React.useState(!initialFaqs || initialFaqs.length === 0);
