...
export function ProductCatalog({ initialProducts, initialCategories }: ProductCatalogProps) {
  ...
  // Seed the client stores from the server-rendered slice, then pull the rest of the
  // catalog in the background — filtering, sorting and infinite scroll all run on the
  // client, so they need the complete list to be correct.
  React.useEffect(() => {
    ...
    <div className="mx-auto max-w-8xl px-4 md:px-6 py-8 text-foreground w-full">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4 border-b pb-6 border-border/60">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Industrial products directly sourced from verified factory partners.</p>
        </div>
