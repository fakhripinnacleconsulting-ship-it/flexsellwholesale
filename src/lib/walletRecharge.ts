...
import { nextReceiptNumber, getOrCreateWallet } from "./walletLedger";
import type { WalletType } from "./walletConstants";
import type { WalletActor } from "@/types/wallet";

/**
 * Settling a wallet recharge — shared by the Razorpay webhook and the browser callback.
 *
...
// A frozen or closed wallet is deliberately absent from this union: it throws rather than
...
/**
 * Credits a wallet against a payment Razorpay has confirmed.
 *
 ...
 * both pass, and the wallet is credited twice.
 */
export async function settleWalletRecharge(input: SettleInput): Promise<RechargeSettlement> {
  ...
  if (capturedPaise !== pending.amount) {
    console.error(
      `[Wallet] Amount mismatch on ${razorpayOrderId} (${source}): expected ${pending.amount} paise, ` +
  ...
  return runInTransaction(async (session) => {
    ...
    if (!wallet) {
      ...
      throw new Error(
        `Wallet ${pending.walletId} is not active; recharge ${razorpayPaymentId} could not be credited`
      );
    }
    ...
    await WalletTransaction.updateOne(
      { _id: claimed._id },
      {
        $set: {
          status: "success",
          receiptNumber,
          balanceBefore: wallet.availableBalance - pending.amount,
...
export async function createPendingRecharge(params: {
  ...
  if (wallet.status !== "active") {
    throw new Error(`This wallet is ${wallet.status} and cannot be recharged.`);
  }
