...
import { permissionsForAdvanceBalance, type AdvanceBalanceType } from "./advanceBalanceConstants";
...
/**
 * Authorises a staff Advance Balance write and returns the actor to attribute it to.
 *
 ...
 * the only thing that grants access is the exact permission for the Advance Balance being written.
 *
 * The permission is derived from `walletType`, never from the request body, so a
 * `wallet_business` holder cannot reach a Store Advance Balance by changing a field in the payload.
 *
 ...
 */
export async function requireAdvanceBalanceSpendAccess(walletType: AdvanceBalanceType): Promise<GuardResult> {
  try {
    const payload = await readSession();
    if (!payload) return unauthorised("Not authenticated", 401);

    if (payload.role === "admin") {
      return { payload, actor: resolveActor(payload) as AdvanceBalanceActor };
    }
    ...
    if (payload.role === "manager") {
      ...
      /**
       * Exact match against the current id **or** the one it replaced.
       *
       * Still no root-permission widening, no wildcard: the widening in
       * requireAdminOrManagerAuth exists so "orders" implies "orders:update", and there is no
       * shorthand that should ever imply "may spend someone's money". The legacy id is
       * accepted because it is what is stored on managers today — see the note on
       * LEGACY_ADVANCE_BALANCE_PERMISSIONS.
       */
      const accepted = permissionsForAdvanceBalance(walletType);
      const held = manager.permissions || [];
      if (!accepted.some((p) => held.includes(p))) {
        return unauthorised("Forbidden: AdvanceBalance permission required", 403);
      }

      return { payload, actor: resolveActor(payload, manager.name) as AdvanceBalanceActor };
    }
...
/**
 * Authorises an admin-only Advance Balance action — the ones that create, return or move money.
 *
 ...
 * delegable at all.
 */
export async function requireAdvanceBalanceAdmin(): Promise<GuardResult> {
  try {
    const payload = await readSession();
    if (!payload) return unauthorised("Not authenticated", 401);
...
/**
 * Authorises a *read* of someone's Advance Balance: the owner, or any staff member.
 *
 ...
 */
export async function requireAdvanceBalanceRead(
  ...
  try {
     ...
     *
     * A customer reading their own Advance Balance sends no `userId` — there is nothing for the
    ...
    if (!requestedSomeoneElse) {
      // Staff have no Advance Balance of their own, so an unqualified read from staff is a mistake
      // rather than a request to see something.
      if (isStaff) {
        return unauthorised("Specify which customer's Advance Balance to view", 400);
      }
      return { payload, actor: resolveActor(payload) as AdvanceBalanceActor };
    }
