/**
 * Every wallet number, key and label that more than one file needs.
 *
...
export const WALLET_TYPES = ["store", "business"] as const;
 ...
 * which is why the wallet has its own guard rather than reusing that one.
 */
export const WALLET_PERMISSIONS: Record<WalletType, string> = {
  store: "wallet_store",
...
export const WALLET_RECEIPT_PREFIX = {
  credit: "WR",
  debit: "WD",
} as const;
 ...
 *
 * A B2C retail buyer has no use for GST registration or ad spend, so the Business Wallet
 * is not offered to them at all.
 */
export const BUSINESS_WALLET_TIERS = ["B2B", "Dropshipping"] as const;
 ...
 */
export const WALLET_TERMS_VERSION = "2026-08-15.1";

export const WALLET_TERMS_TEXT =
  "Wallet balance can be used for purchases and services on FlexSell only. " +
...
/**
 * Why online wallet top-up is unavailable, in words for each audience.
 *
...
export const RECHARGE_UNAVAILABLE_MESSAGE: Record<string, string> = {
  disabled_by_admin:
    "Online wallet top-up is currently unavailable. Please contact us to add funds by bank transfer, UPI or cash.",
  gateway_not_configured:
    "Online wallet top-up is temporarily unavailable. Please contact us to add funds another way.",
};
...
export const RECHARGE_UNAVAILABLE_STAFF_MESSAGE: Record<string, string> = {
  disabled_by_admin:
    "Online wallet top-up is switched off in Settings → Commerce. Use Add Funds Offline, or turn it back on.",
  gateway_not_configured:
