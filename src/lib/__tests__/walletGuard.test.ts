...
/**
 * The wallet guard is the boundary that decides who may move a customer's money. With no
...
describe("requireWalletSpendAccess", () => {
  ...
  it("does not widen a root permission into wallet access", async () => {
    verifyToken.mockReturnValue({ userId: "M1", role: "manager" });
    managerFindById.mockReturnValue(
  ...
  it("refuses a customer outright", async () => {
    ...
    const result = await requireWalletSpendAccess("store");
...
describe("requireWalletAdmin", () => {
  it("refuses a manager even with every wallet permission", async () => {
    // Creating and returning money is not delegable at all — not by permission, not ever.
    verifyToken.mockReturnValue({ userId: "M1", role: "manager" });

    const result = await requireWalletAdmin();
...
describe("requireWalletRead", () => {
  /**
   * The regression this suite exists for.
   *
   * A customer reading their own wallet sends no `userId`. An earlier version compared
   * `payload.userId` against an empty string — never equal — so every customer got a 403 on
   * their own wallet page and the whole feature was unreachable.
   */
  it("lets a customer read their own wallet when no userId is given", async () => {
    verifyToken.mockReturnValue({ userId: "CUST-1", role: "customer" });

    for (const target of [undefined, null, ""]) {
      const result = await requireWalletRead(target as string | null | undefined);
      expect(result.error, `target ${JSON.stringify(target)} should be allowed`).toBeUndefined();
      expect(result.payload!.userId).toBe("CUST-1");
    }
  });

  it("lets a customer read their own wallet when the id is passed explicitly", async () => {
    verifyToken.mockReturnValue({ userId: "CUST-1", role: "customer" });

    const result = await requireWalletRead("CUST-1");
  ...
  it("refuses a customer reading someone else's wallet", async () => {
    ...
    const result = await requireWalletRead("CUST-2");
  ...
  it("lets staff read any customer's wallet", async () => {
    for (const role of ["admin", "manager"]) {
      ...
      const result = await requireWalletRead("CUST-9");
      expect(result.error, `${role} should be allowed`).toBeUndefined();
    }
  });

  it("asks staff which wallet they meant rather than guessing", async () => {
    // Staff hold no wallet of their own, so an unqualified read is a mistake — and falling
    // through to `payload.userId` would query a wallet that cannot exist.
    verifyToken.mockReturnValue({ userId: "S1", role: "admin" });

    const result = await requireWalletRead();
