...
vi.mock("@/models/AdvanceBalance", () => ({
  default: {
    findOneAndUpdate: (...args: unknown[]) => advanceBalanceFindOneAndUpdate(...args),
  },
}));
...
vi.mock("../advanceBalanceLedger", () => ({
  nextReceiptNumber: vi.fn(async () => "WR-000042"),
  getOrCreateAdvanceBalance: vi.fn(),
}));
...
describe("settleAdvanceBalanceTopUp", () => {
  it("credits the Advance Balance when the captured amount matches", async () => {
    findOne.mockReturnValue(lean(PENDING));
    findOneAndUpdate.mockResolvedValue({ _id: "txn_1" });
    advanceBalanceFindOneAndUpdate.mockResolvedValue({ availableBalance: 3000000 });
    updateOne.mockResolvedValue({});

    const result = await settleAdvanceBalanceTopUp({
    ...
    expect(advanceBalanceFindOneAndUpdate).toHaveBeenCalledOnce();
  });
  ...
  it("throws rather than swallowing the payment when the Advance Balance is frozen", async () => {
    findOne.mockReturnValue(lean(PENDING));
    findOneAndUpdate.mockResolvedValue({ _id: "txn_1" });
  ...
  it("records which path settled it, for later reconciliation", async () => {
    ...
    await settleAdvanceBalanceTopUp({
      razorpayOrderId: "order_x",
      razorpayPaymentId: "pay_x",
      capturedPaise: 3000000,
      source: "sweeper",
  ...
  it("writes balanceBefore and balanceAfter that reconcile against the amount", async () => {
    findOne.mockReturnValue(lean(PENDING));
    findOneAndUpdate.mockResolvedValue({ _id: "txn_1" });
    advanceBalanceFindOneAndUpdate.mockResolvedValue({ availableBalance: 4500000 });
    updateOne.mockResolvedValue({});
